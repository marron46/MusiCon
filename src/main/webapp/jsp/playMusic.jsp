...
<body>
	...
	<div class="reverse">
		<a href="<%=isBookmarkMode ? request.getContextPath() + "/MyBookmark" : request.getContextPath() + "/PlayMusic"%>"><img
			src="${pageContext.request.contextPath}/png/MusiConLogo.png"
			alt="TOPに戻る" class="reverse-img">
		</a>
	</div>
	<div class="reverseStr">
		<a href="<%=isBookmarkMode ? request.getContextPath() + "/MyBookmark" : request.getContextPath() + "/PlayMusic"%>">
		...
		<div class="player">
			...
			<div class="info">
				...
				<form action="${pageContext.request.contextPath}/MyBookmark"
					...
					if (isBookmarkMode) {
					%>
					<input type="hidden" name="bookmarkMode" value="true">
					<input type="hidden" name="bookmarkIndex" value="<%=index%>">
					<%
					}
					%>
					<%
					if (isBookmarked) {
					%>
					<button type="submit" class="like-btn" style="background-color: #f7d358;">
						★ ブックマーク解除
					</button>
					<%
					} else {
					%>
					<button type="submit" class="like-btn" style="background-color: #dddddd;">
						☆ ブックマーク
					</button>
					<%
					}
